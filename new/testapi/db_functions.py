...
def update_userstats(user_id, assessment_id, count):
    try:
        # Get the count (maximum count for the user and assessment)
        # max_count = Responses.objects.filter(user_id=user_id, question__assessment_id=assessment_id).aggregate(max_count=models.Max('count'))['max_count']
